...
PROJECT="jbossdemocentral/filenjdbc.git"
...
echo "Start compile and deploy File and JDBC example project to fuse"
echo         
mvn fabric8:deploy
...
while true; do
    if [ $(sh $FUSE_SERVER_BIN/client 'profile-list'| grep "demo-filenjdbc"| wc -l ) -ge 1 ]; then
        break
    fi
    sleep 2
done
...
echo         
sh $FUSE_SERVER_BIN/client -r 2 -d 40 'container-create-child --profile demo-filenjdbc root filejdbccon'
